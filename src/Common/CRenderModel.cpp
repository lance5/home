...
void CRenderModel::PushObjectData(const std::vector<SVectexData>& vecData, CMaterial * pMaterail, bool bSmooth)
{
	...
	CheckError();
	glVertexAttribPointer( 0, 3, GL_FLOAT, GL_FALSE, sizeof( SVectexData ), (void*)0 );
	CheckError();
	glEnableVertexAttribArray( 0 );
	CheckError();
	glVertexAttribPointer( 1, 3, GL_FLOAT, GL_FALSE, sizeof( SVectexData ), (void*)(3 * sizeof(float)) );
	CheckError();
	glEnableVertexAttribArray( 1 );
	CheckError();
	glVertexAttribPointer( 2, 2, GL_FLOAT, GL_FALSE, sizeof( SVectexData ), (void*)(6 * sizeof(float)) );
	CheckError();
